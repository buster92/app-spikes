...
function postCard(item, surface) {
  ...
  const isSelf = creator.id === service.actorId();
  const view = postPresentation({ post: item, creator, benchmark, liked: service.isLiked(item.id), following: isSelf ? false : service.isFollowing(creator.id), isSelf, mode: presentationMode });
...
function modalView() {
  ...
  const view = resultPresentation({ post: source, creator, playerResult: play.result, benchmark: result(source.creatorResultId), comparison: play.comparison, isSelf: creator?.id === service.actorId(), mode: presentationMode, persisted: play.persisted });
