...
export const DEFAULT_REVIEW_SEEDS = Object.freeze([1, 7, 42, 99, 1337]);
...
export function reviewWithRuntime(spec, adapter, options = {}) {
  const profileSpec = adapter?.profileSpec;
  const validatePublication = adapter?.validatePublication;
  const RuntimeClass = adapter?.RuntimeClass;
  const limits = adapter?.limits || HARD_LIMITS;
  const runtimeLabel = adapter?.runtimeLabel || spec?.runtime || "unknown";

  if (typeof profileSpec !== "function" || typeof validatePublication !== "function" || typeof RuntimeClass !== "function") {
    throw new Error("reviewWithRuntime requires profileSpec, validatePublication and RuntimeClass");
  }

  let profile;
  try {
    profile = profileSpec(spec);
  } catch (error) {
    return rejectStatic(
      null,
  ...
  const publication = validatePublication(spec);
  if (!publication.ok) {
  ...
  const stepMs = clamp(Number(options.stepMs || 50), 16, limits.maxStepMs);
  const maxSimulatedMs = clamp(
    Number(options.maxSimulatedMs || Math.min(limits.maxDurationMs, 15_000)),
    stepMs,
  ...
  if (peakEntities >= Math.floor(limits.maxEntities * 0.9)) {
    warnings.push("Runtime entity usage reaches at least 90% of the hard limit.");
  }
  if (peakOps >= Math.floor(limits.maxOpsPerStep * 0.9)) {
