import { ASSET_POLICY, decodedImageBytes } from "./asset-contract.js";
...
function collectUsedActionAssets(actions, output = new Set()) {
  for (const action of actions || []) {
    if (!isObject(action)) continue;
    if (typeof action.sound?.asset === "string") output.add(action.sound.asset);
    if (isObject(action.if)) {
      collectUsedActionAssets(action.if.then, output);
      collectUsedActionAssets(action.if.else, output);
    }
  }
  return output;
}
...
function validateEntityFlags(entity, path, errors) {
  for (const key of ["collidable", "interactive"]) {
    if (entity?.[key] !== undefined && typeof entity[key] !== "boolean") errors.push(`${path}.${key}: must be a boolean`);
  }
}
...
export function validatePublicationPolicy(spec) {
  ...
  for (const [index, asset] of (Array.isArray(spec.assets) ? spec.assets : []).entries()) {
    ...
    totalDecodedImageBytes += decodedImageBytes(asset);
  }
  if (totalDecodedImageBytes > ASSET_POLICY.maxTotalDecodedImageBytes) {
    errors.push(`assets: decoded raster working set ${totalDecodedImageBytes} exceeds ${ASSET_POLICY.maxTotalDecodedImageBytes} bytes`);
  }
  ...
  for (const [id, template] of Object.entries(isObject(spec.templates) ? spec.templates : {})) {
    ...
    validateEntityFlags(template, `templates.${id}`, errors);
  ...
  for (const [index, entity] of (Array.isArray(spec.entities) ? spec.entities : []).entries()) {
    ...
    validateEntityFlags(entity, `entities[${index}]`, errors);
  ...
  for (const rule of Array.isArray(spec.rules) ? spec.rules : []) {
    collectFixedSpawnIds(rule?.actions, fixedSpawnIds);
    collectUsedActionAssets(rule?.actions, usedAssetIds);
  }
  ...
  return {
    ...
    errors,
    warnings,
    metrics: { totalDecodedImageBytes },
  };
}
