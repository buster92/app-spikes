...
export function validateReplayTrace(spec, trace) {
  ...
  if (trace.runtime !== RUNTIME_ID) errors.push(`runtime: must be ${RUNTIME_ID}`);
  ...
  if (!Number.isInteger(trace.durationMs) || trace.durationMs < 0 || trace.durationMs > HARD_LIMITS.maxDurationMs) {
    errors.push(`durationMs: must be 0-${HARD_LIMITS.maxDurationMs}`);
  }
...
export function runReplay(spec, trace) {
  ...
  const runtime = new SafeSandboxRuntime(spec, { seed: trace.seed });
  runtime.start();
  ...
  const advanceTo = (targetMs) => {
    while (runtime.status === "running" && runtime.elapsedMs < targetMs) {
      runtime.step(Math.min(HARD_LIMITS.maxStepMs, targetMs - runtime.elapsedMs));
    }
  };
